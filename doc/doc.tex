...
\end{figure}
    ...
    \begin{enumerate}
        ...
        \itemsep1pt\parskip0pt\parsep0pt
        \item Inicializujte a spusťte docker kontejnery:

        \begin{itemize}
            \itemsep1pt\parskip0pt\parsep0pt

            \item Spusťte kontejnery:
            ...
            \item Spusťte kontejnery na pozadí:
        ...
        \item Počkejte, až budou všechny kontejnery spuštěné a následně inicializujte \texttt{superset} a vytvořte uživatele administrátora:
        ...
        \item Inicializujte databázi a dashboardy:

        \begin{itemize}
            \itemsep1pt\parskip0pt\parsep0pt
            \item[] \texttt{make restore-superset}
        \end{itemize}
    \end{enumerate}

    \subsection{Zastavení služeb}

    K zastavení všech služeb spusťte příkaz: \texttt{make down}

    \subsection{Správa dat}

    Pro přidání dat do NoSQL databáze vytvořte adresář \texttt{pocasi} a zkopírujte do něj dataset. Následně spusťte příkaz: \texttt{make scrape}

    Aktualizaci SQL databáze daty získanými z NoSQL databáze provedete příkazem: \texttt{make compute}

    \subsection{Správa uživatelů}

    Vytvoření uživatelů a administrátorů lze docílit dvěma způsoby -- buď v grafickém rozhraní Superset (pokud již byl alespoň jeden administrátorský účet vytvořen) nebo přes příkazovou řádku.

    Přidání administrátora přes příkaz:  \texttt{make create-admin}

    Přidání uživatele přes příkaz: \texttt{make create-user}

    \subsection{Nápověda}

    Pro zobrazení nápovědy použijte příkaz: \texttt{make help}
