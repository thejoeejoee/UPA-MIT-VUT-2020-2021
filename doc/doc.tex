...
\end{figure}
    ...
    bez měřících stanic.}
    Z dotazu C se podařilo vizualizovat jenom průměrnou teplotu se podařila. Interpolace by se dala implementovat například následovně: 
    
    \begin{enumerate}
        \item vytvořit matici $latitude \times longitude$ pro celou Austrálii s jemností dle požadavků
        \item pro pole se stanicí udělat agregaci a uložit si pro pole \texttt{avg} a \texttt{stdev}
        \item pro chybějící pole se podívat na okolní, zprůměrovat \texttt{mean} a \texttt{stdev} a vždy začínat podle počtu okolních, čím více vyplněných, tím dřive interpolovat
        \item interpolovat hodnoty vygenerovaním z \texttt{avg\_mean} a \texttt{avg\_stdev}, tedy z normálního rozdělení
        \item rekurzivně opakovat dokud nejsou všechna pole naplněna hodnotami
        \item výstupem jsou \texttt{mean} z každého pole
    \end{enumerate}
